...
export async function updateAPI<T>(
  ...
  await updateDoc(
    doc(getFirestore(), docName, id) as DocumentReference<T>,
    data
  );
}
...
export async function getBestPlayers(sortBy: string) {
  ...
  const querySnapshot = await getDocs(
    query(collection(getFirestore(), "users"), orderBy(sortBy), limitToLast(5))
  );
  querySnapshot.forEach((doc) => {
    docs.push(doc);
  });
  return docs.map((bestPlayers) => ({
    ...bestPlayers.data(),
    id: bestPlayers.id,
  }));
}
...
export async function getPlayer(): Promise<type.User | null> {
  const id = getAuth().currentUser?.uid;
  return id
    ? ((await getDoc(doc(getFirestore(), "users", id))).data() as type.User)
    : null;
}
...
export async function getFleetMembers(
  ...
  const querySnapshot = await getDocs(
    query(
      ...
      where("player.spaceFleet", "==", spaceFleet)
    )
  );
  querySnapshot.forEach((doc) => {
    docs.push(doc);
  });
  return docs.map((members) => ({
    ...(members.data() as type.User),
    id: members.id,
  }));
}
export async function searchSpaceFleet(name: string) {
  ...
  const querySnapshot = await getDocs(
    query(
      collection(getFirestore(), "spaceFleets"),
      where("name", "<=", name + "~"),
      where("name", ">=", name)
    )
  );
...
export async function lvlFleetSkill(
  fleetID: string,
  skills: type.FleetSkill[]
) {
  await updateDoc(doc(getFirestore(), "spaceFleets", fleetID), {
    skills: skills,
  });
}
export async function updateFleetInfo(
  fleetID: string,
  fleetInfo: type.FleetInfo
) {
  await updateDoc(doc(getFirestore(), "spaceFleets", fleetID), {
    fleetInfo: fleetInfo,
  });
