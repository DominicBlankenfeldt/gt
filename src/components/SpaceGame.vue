    ...
    <div v-if="dataLoad" style="color: #18191a">
      ...
      <div class="row g-0" style="height: 5vh; position: relative">
        <div v-if="!bossEnemy.type" class="col-3">
          ...
          Score:
          {{ score > 1000000 ? Math.round(score / 1000) + 'k' : Math.round(score) }}+{{ (scorePerTick * 60).toFixed(0) }}/s
          <img src="../../public/img/items/coin/coin.gif" alt="coin" />
        </div>
      ...
      <div>
        <div class="d-flex justify-content-between">
          ...
          <div class="game" :class="{ noneCursor: gameStarted }">
            ...
            >
              <img
                :src="`/img/char/playership${player.ship.selectedModel.img}.png`"
                alt=""
                :style="{
                  width: playerInfo.size + 'px',
                  height: playerInfo.size + 'px',
            ...
            <div class="container" v-if="!gameStarted">
              ...
              <br />
              <button class="btn shadow-none" keydown.enter @click="changePlayMode('normal')" v-if="!cancelButtonText">
                <a>
                  normal:
                  <br />
              ...
              <button class="btn shadow-none" keydown.enter @click="changePlayMode('hardcore')" v-if="!cancelButtonText">
                <a>
                  hardcore:
                  <br />
              ...
              <button class="btn shadow-none" keydown.enter @click="changePlayMode('totalchaos')" v-if="!cancelButtonText">
                <a>
                  total chaos:
                  <br />
      ...
      <div class="mt-3">
        ...
        <div v-if="gameStarted" class="d-flex justify-content-center">
          ...
          <div v-for="hp of shield" :key="hp" style="color: grey">
            <svg xmlns="http://www.w3.org/2000/svg" width="2vw" height="2vw" fill="currentColor" class="bi bi-shield-fill" viewBox="0 0 16 16">
              <path
              ...
              <div v-if="currentUser">
                <div v-for="shopItem of ['lessStartEnemies', 'higherDifficultyTimer', 'lowerScoreTimer']" :key="shopItem" class="mt-1">
                  <label
                    ...
                    "
                    :style="{
                      backgroundColor: player.shop[shopItem].use ? 'green' : 'red',
                    }"
  ...
  methods: {
    ...
    setSkillPoints() {
      if (this.player.skillTree.skillPoints < Math.floor(this.player.highscore['normal'] / 1000)) {
        this.receiveMessages.push(`you get ${Math.floor(this.player.highscore['normal'] / 1000) - this.player.skillTree.skillPoints} skillpoints`);
    ...
    bossEnemyPreparations() {
      ...
      this.bossEnemy.imgsrc = '/img/boss/bossEnemy.gif';
        ...
        case 'normal':
          for (let i = 0; i < 7; i++) {
            this.enemyPlasmas.push({
              ...
              imgsrc: '/img/char/enemy_plasma.png',
              damage: 1,
            });
          }
          break;
    ...
    spawnItems(badItems: boolean) {
      this.items =
        createItems(this.effects.stopTime.active, this.generalSize, this.player, this.items, this.field, badItems, this.bossFight, this.playerInfo) ||
        this.items;
    },
    ...
    collectGrowPotion(item: type.Item) {
      ...
      this.effects.grow = {
        active: true,
        duration: (this.effects.grow.duration += ((250 * item.size) / this.generalSize) * percent(this.passivObject['longerEffects'] / 3, 'in')),
      };
    ...
    handlePlasmaMovement() {
      const newPlasmas = plasmaMovement(this.plasmas, this.enemyPlasmas, this.enemies, this.player, this.generalSize, this.field, this.bossEnemy);
...
.game {
  background-image: url(/img/spacefield.png);
