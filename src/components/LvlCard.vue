<template>
  <div v-if="dataLoad" style="margin-top: 6vh; color: white">
    <div>
      <div
        data-title="you can lvl up by make the daily tasks"
        class="w-25 d-inline"
      >
    ...
    <div class="row g-0 mt-2">
        ...
        <div v-for="skill of player.lvlTree.lvlSkills" :key="skill.name">
          <button
            ...
            :data-title="
              ...
              '\n' +
              (skill.lvl < lvlSkillsDetails[skill.name].maxlvl
                ? `costs: 1`
                : 'max lvl')
            "
          >
      ...
      <div class="d-flex flex-column col-2 mx-1">
        <div
          v-for="lvlWeaponUpgrade of player.lvlTree.lvlWeaponUpgrade"
          :key="lvlWeaponUpgrade.name"
        >
          <button
            @click="lvlUpgrade(lvlWeaponUpgrade)"
            :disabled="
              lvlWeaponUpgrade.name == 'tier2' &&
              findlvlWeaponUpgrade(player, 'tier1') < 5
            "
            class="mt-2 w-100 btn btn-primary align-self-center shadow-none"
            :data-title="
              lvlWeaponUpgradeDetails[lvlWeaponUpgrade.name].description +
              '\n' +
              (lvlWeaponUpgrade.lvl <
              lvlWeaponUpgradeDetails[lvlWeaponUpgrade.name].maxlvl
                ? `costs: 1`
                : 'max lvl')
            "
          >
        ...
        <div v-for="passiv of player.lvlTree.lvlPassiv" :key="passiv.name">
          <button
            ...
            :data-title="
              ...
              '\n' +
              (passiv.lvl < lvlPassivDetails[passiv.name].maxlvl
                ? `costs: 1`
                : 'max lvl')
            "
          >
        ...
        <div v-for="shop of player.lvlTree.lvlShop" :key="shop.name">
          <button
            ...
            :data-title="
              ...
              '\n' +
              (shop.lvl < lvlShopDetails[shop.name].maxlvl
                ? `costs: 1`
                : 'max lvl')
            "
          >
...
export default defineComponent({
  ...
  computed: {
    usedLvl() {
      let allPassivlvl = 0;
      for (let lvlSkill of this.player.lvlTree.lvlSkills)
        allPassivlvl += lvlSkill.lvl;
      for (let lvlWeaponUpgrade of this.player.lvlTree.lvlWeaponUpgrade)
        allPassivlvl += lvlWeaponUpgrade.lvl;
      for (let lvlPassiv of this.player.lvlTree.lvlPassiv)
        allPassivlvl += lvlPassiv.lvl;
      for (let lvlShop of this.player.lvlTree.lvlShop)
        allPassivlvl += lvlShop.lvl;
      return allPassivlvl;
    },
  },
  mounted() {
