<template>
    ...
    <div style="font-size: 4vw"><u>hardcore</u></div>
    <div class="row g-0">
        <div class="col-12 row g-0">
            ...
            <div>{{ Math.round(bestPlayers[0]?.player.highscoreHardcore) }}</div>
        </div>
        <div class="col-12 row g-0 justify-content-between">
            <div class="col-4">
                ...
                <div>{{ Math.round(bestPlayers[1]?.player.highscoreHardcore) }}</div>
            </div>
...
export default defineComponent({
    ...
    async mounted() {
        let result = await API.getBestPlayers('player.highscoreHardcore')
        if (result) {
