...
export function plasmaMovement(
  ...
  for (const plasma of plasmas) {
    if (plasma.aim) {
      ...
      enemies.sort((a, b) => {
        return (
          lenVecSqrt(subVec(a.vector, plasma.vector)) -
          lenVecSqrt(subVec(b.vector, plasma.vector))
        );
      });
      plasma.moveVector = addVec(
        plasma.moveVector,
        dirVec(enemies[0].vector, plasma.vector)
      );
    }
    plasma.moveVector = mulVec(
      ...
      (7 + findWeaponUpgrade(player, "fasterProjectile") / 2) *
        generalSize *
        (player.passivTree.passivType.includes("increaseGun")
          ? percent(findPassivUpgrade(player, "increaseGun") / 3, "in")
          : 1)
    );
    plasma.vector = addVec(plasma.vector, plasma.moveVector);
    if (borderCheck(plasma, "outer", field))
      plasmas = deletePlasma(plasmas, plasma);
  }
...
export function playerMovement(
  ...
  if (pressedKeys["ArrowLeft"] || pressedKeys[player.settings.moves["left"]])
    playerInfo.moveVector[0] = left(playerInfo, field);
  if (pressedKeys["ArrowRight"] || pressedKeys[player.settings.moves["right"]])
    playerInfo.moveVector[0] = right(playerInfo, field);
  if (pressedKeys["ArrowUp"] || pressedKeys[player.settings.moves["up"]])
    playerInfo.moveVector[1] = up(playerInfo, field);
  if (pressedKeys["ArrowDown"] || pressedKeys[player.settings.moves["down"]])
    playerInfo.moveVector[1] = down(playerInfo, field);

  playerInfo.moveVector = mulVec(
    norVec(playerInfo.moveVector),
    playerInfo.speed * generalSize * multiplicator
  );
  ...
  if (playerInfo.moveVector[0] > 0 && playerInfo.moveVector[1] > 0)
    lastDirection = 135;
  if (playerInfo.moveVector[0] < 0 && playerInfo.moveVector[1] > 0)
    lastDirection = 225;
  if (playerInfo.moveVector[0] > 0 && playerInfo.moveVector[1] < 0)
  ...
  for (const enemy of enemies) {
    if (enemy.isFear) {
      ...
      if (enemy.type == "circle")
        enemy.moveVector = moveCircleEnemy(enemy, difficulty);
      if (enemy.type == "curve") enemy.moveVector = moveCurveEnemy(enemy);
      if (enemy.type == "chasebot") {
        enemy.vector = moveChasebotEnemy(
          enemy,
          player,
          generalSize,
          isSlowEnemies,
        ...
        enemy.vector = addVec(
          ...
          mulVec(
            ...
            difficulty *
              ...
              generalSize *
              (isSlowEnemies
                ? 0.75 - skillObject["strongerSlowEnemies"] / 150
                : 1) *
              (player.passivTree.passivType.includes("nerfEnemies")
                ? percent(findPassivUpgrade(player, "nerfEnemies") / 4, "de")
                : 1) *
              enemy.speed
          )
        );
    ...
    if (borderCheck(enemy, "outer", field))
...
function circle(enemy: type.Enemy, radiusMultiplier: number) {
  const acc = mulVec(
    rotVec(enemy.moveVector, 90),
    enemy.circleRadius * radiusMultiplier
  );
