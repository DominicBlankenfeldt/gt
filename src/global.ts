...
export function checkPlayer(player: type.Player, user?: User) {
  ...
  for (const weaponUpgrade of Object.entries(weaponDetails)
    ...
    if (checkWeaponUpgrade(player, weaponUpgrade)) {
      player.weaponTree.weaponUpgrades.push({
        name: weaponUpgrade as type.WeaponUpgradeName,
        lvl: 0,
      });
    }
  }
  ...
  for (const passivUpgrade of Object.keys(passivDetails)) {
    if (checkPassivUpgrade(player, passivUpgrade)) {
      player.passivTree.passivUpgrades.push({
        name: passivUpgrade as type.PassivUpgradeName,
        lvl: 0,
      });
    }
  }
  ...
  for (const house of Object.keys(houseDetails)) {
    if (checkHouse(player, house)) {
      player.spaceport.houses.push({
        name: house as type.HouseName,
        lvl: houseDetails[house as type.HouseName].startlvl,
        needScore: 0,
      });
    }
  }
  ...
  for (const lvlSkill of Object.keys(lvlSkillsDetails)) {
    if (checkLvlSkill(player, lvlSkill)) {
      player.lvlTree.lvlSkills.push({
        name: lvlSkill as type.LvlSkillName,
        lvl: 0,
      });
    }
  }
  ...
  for (const lvlWeaponUpgrade of Object.keys(lvlWeaponUpgradeDetails)) {
    if (checkLvlWeaponUpgrade(player, lvlWeaponUpgrade)) {
      player.lvlTree.lvlWeaponUpgrade.push({
        name: lvlWeaponUpgrade as type.LvlWeaponUpgradeName,
        lvl: 0,
      });
    }
  }
  ...
  for (const lvlPassiv of Object.keys(lvlPassivDetails)) {
    if (checkLvlPassiv(player, lvlPassiv)) {
      player.lvlTree.lvlPassiv.push({
        name: lvlPassiv as type.LvlPassivName,
        lvl: 0,
      });
    }
  }
  ...
  for (const lvlShop of Object.keys(lvlShopDetails)) {
    if (checkLvlShop(player, lvlShop)) {
      player.lvlTree.lvlShop.push({
        name: lvlShop as type.LvlShopName,
        lvl: 0,
      });
    }
  }
